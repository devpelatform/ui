...
import { cn, cva, type VariantProps } from '@pelatformui/general';
import {
  Command,
  CommandEmpty,
  CommandGroup,
...
const flattenFields = <T = unknown>(fields: FilterFieldsConfig<T>): FilterFieldConfig<T>[] => {
  return fields.reduce<FilterFieldConfig<T>[]>((acc, item) => {
    // biome-ignore-start lint/performance/noAccumulatingSpread: disabled for performance reasons
    ...
    return [...acc, item];
    // biome-ignore-end lint/performance/noAccumulatingSpread: disabled for performance reasons
  }, []);
};
