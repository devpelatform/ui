...
export const getMask = ({ direction = 'circle at center', visibleRadius = 30, fadeWidth = 20 }) => {
...
export const getDotPattern = ({
  ...
  return {
    backgroundImage: `radial-gradient(${dotColor} ${size}%, transparent ${
      size + 1
    }%), radial-gradient(${dotColor} ${size - 0.5}%, transparent ${size + 0.5}%)`,
...
export function getPositionClasses(element: PositionedElement, maxWidth: number) {
