    ...
    if (loc) {
      ...
      if (lineParts[0]?.startsWith('L')) {
        ...
        if (fromLine !== undefined && toLine !== undefined) {
          ...
          highlightLines = startLine === endLine ? `{${startLine}}` : `{${startLine}-${endLine}}`;
        }
      }
    }
...
async function fetchCode(url: string, fromLine?: number, toLine?: number) {
  try {
    ...
    const commonIndent = selectedLines.reduce((indent: number, line: string) => {
...
async function CodeBlock({ code, lang, wrapper, highlightLines }: CodeBlockProps) {
...
export async function GithubCodeBlock({
  url,
  extractLines = false,
  highlightLines,
  wrapper,
